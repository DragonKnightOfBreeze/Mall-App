...
let url = null;// 请求远程数据的地址
let condition = null;// 文本框的值，作为查询条件使用
let imgName = null; // 图片文本框的值，判断图片有没有更新
...
function searchRecord() {
  $('#dg_guide').datagrid({
    ...
    success: function(data) {
      $("#dg_guide").datagrid("loadData", data.rows);
    }
  });
}
...
$("#btn_Edit").click(function() {
  url = "/MallWeb/guidePage/update";
  let row = $("#dg_guide").datagrid("getSelected");
  if(row == null) {
  ...
  $("#adRight").textbox('setValue', row.adRight);
...
function saveData() {
  let id = $("#id").val();
  let adTitle = $("#adTitle").textbox("getValue");
  let adLeft = $("#adLeft").textbox("getValue");
  let adRight = $("#adRight").textbox("getValue");
  let page = $("#page").textbox("getValue");
...
let token = null;// 保存token
let key = null;// 保存上传的文件名
let timestamp = null;// 时间戳，用于重命名key
// 配置七牛上传和plupload
let uploader = Qiniu.uploader({
  ...
  domain: 'http://pr27me2rw.bkt.clouddn.com/', // bucket域名，下载资源时用到，必需
  ...
  uptoken_func: function(file) { // 将key传给服务端获取的七牛token，上传后覆盖同名文件，即为更新操作
    let val = $("#imgUrl").val();// val用于保存上传文件的url
